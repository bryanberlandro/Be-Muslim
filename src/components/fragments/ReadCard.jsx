...
export function ReadCard({title, name, value, position}){
    return(
        <>
        <div className="bg-white px-5 py-5 relative rounded-xl shadow-xl mx-auto w-[90%] md:w-[80%] md:px-7 md:py-7">
                <div className="flex justify-between border-b-2 pb-4 border-emerald-100 text-sm">
                    <h1>{title}</h1>
                    <h1 className="font-medium">{name}</h1>
                </div>
                <div className={`py-6 flex justify-between ${position} font-medium  items-center md:py-7`}>
                    <p>{value}</p>
                </div>
